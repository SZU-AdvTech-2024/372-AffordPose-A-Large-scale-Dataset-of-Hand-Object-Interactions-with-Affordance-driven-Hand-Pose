...
class Trainer:
    ...
    def __init__(self,cfg, inference=False):
        ...
        with torch.no_grad():
            ...
            self.rhm_train = Hand_mano_graspit.HandManoGraspIt(cfg.rhm_path, 
                                                          'obj',
                                                          scale=1, 
                                                          use_joint_limit=False, 
                                                          use_quat=True, 
                                                          use_eigen=False).to(self.device)
            
            rhm_train_rn = Hand_mano_graspit.HandManoGraspIt(cfg.rhm_path, 
                                                          'obj',
                                                          scale=1, 
                                                          use_joint_limit=False, 
                                                          use_quat=True, 
        ...
        self.refine_net.cfg = cfg
        self.refine_net.device = self.device
        ...
        # jjt:
        rh_f = np.load(cfg.faces_path, allow_pickle=True)
        rh_f = torch.from_numpy(rh_f.astype(np.int32)).view(1, -1, 3)
        self.rh_f = rh_f.repeat(self.cfg.batch_size,1,1).to(self.device).to(torch.long)
    ...
    def train(self):
        ...
        for it, dorig in enumerate(self.ds_train):
            ...
            dorig = {k: dorig[k].to(self.device).to(torch.float32) for k in dorig.keys()}
            ...
            if self.fit_cnet:
                ...
                # jjt: Clips gradient
                torch.nn.utils.clip_grad_norm_(self.coarse_net.parameters(), max_norm=5)
    ...
    def evaluate(self, ds_name='val'):
        ...
        with torch.no_grad():
            for dorig in data:
                ...
                dorig = {k: dorig[k].to(torch.float32).to(self.device) for k in dorig.keys()}
    ...
    def params_rnet(self,dorig):
        rh_mesh = Meshes(verts=dorig['verts_rhand_f'], faces=self.rh_f).to(self.device).verts_normals_packed().view(-1, 946, 3)
        rh_mesh_gt = Meshes(verts=dorig['verts_rhand'], faces=self.rh_f).to(self.device).verts_normals_packed().view(-1, 946, 3)
    ...
    def loss_rnet(self, dorig, drec, ds_name='train'):
        ...
        out_put = getHandMesh(self.cfg.rhm_path, self.rhm_train.palm.forward(**drec))
        verts_rhand = out_put.to(self.dtype).to(self.device)
        # verts_rhand = out_put.vertices

        rh_mesh = Meshes(verts=verts_rhand, faces=self.rh_f).to(self.device).verts_normals_packed().view(-1, 946, 3)
        h2o_gt = dorig['h2o_gt']
    ...
    def loss_cnet(self, dorig, drec, ds_name='train'):
        ...
        # jjt:half+
        # loss_mesh_rec_w = 200 * (1. - self.cfg.kl_coef) * torch.mean(torch.einsum('ijk,j->ijk', torch.abs((dorig['verts_rhand'] - verts_rhand)), self.v_weights))
        ########## edge loss
        ...
        ########## dof_loss
        loss_dof = (1. - self.cfg.kl_coef) * self.LossL2(dorig['fpose_rhand_dof'].to(self.dtype).to(self.device), drec['dofs'])
...
#################################### jjt: θ loss###############################################
        # loss_dof = 5*(1. - self.cfg.kl_coef) * self.LossL2(dorig['fpose_rhand_dof'].to(self.dtype).to(self.device), drec['dofs'])
        # loss_trans = 0.001*(1. - self.cfg.kl_coef) * self.LossL2(dorig['trans_rhand'].to(self.dtype).to(self.device), drec['trans'])
        # loss_global = 5*(1. - self.cfg.kl_coef) * self.LossL2(dorig['global_orient_rhand_rotmat'].to(self.dtype).to(self.device), drec['global_orient'])
        # ########## KL loss
        # p_z = torch.distributions.normal.Normal(
        #     loc=torch.tensor(np.zeros([self.cfg.batch_size, self.cfg.latentD]), requires_grad=False).to(device).type(dtype),
        #     scale=torch.tensor(np.ones([self.cfg.batch_size, self.cfg.latentD]), requires_grad=False).to(device).type(dtype))
        # loss_kl = 20*self.cfg.kl_coef * torch.mean(torch.sum(torch.distributions.kl.kl_divergence(q_z, p_z), dim=[1]))
        
        # loss_dict = {"loss_kl": loss_kl,
        #              'loss_dof': loss_dof,
        #              'loss_trans': loss_trans,
        #              'loss_global': loss_global,
        #              'loss_mesh_rec_w': loss_mesh_rec_w
        #              }
    ...
    def fit(self, n_epochs=None, message=None):
        ...
        early_stopping_cnet = EarlyStopping(patience=50, trace_func=self.logger)
        early_stopping_rnet = EarlyStopping(patience=50, trace_func=self.logger)
        ...
        for split, ds in [('val', self.ds_val), ('test', self.ds_test), ('train', self.ds_train)]:
            ...
            with torch.no_grad():
                for dorig in ds:
                    ...
                    dorig = {k: dorig[k].to(self.dtype).to(self.device) for k in dorig.keys()}

                    MESH_SCALER = 1000

                    drec_cnet = self.coarse_net(**dorig)
                    # verts_hand_cnet = self.rhm_train(**drec_cnet).vertices
                    verts_hand_cnet = getHandMesh(self.cfg.rhm_path, self.rhm_train.palm.forward(**drec_cnet)).to(self.dtype).to(self.device)
                    ...
                    verts_hand_mano = getHandMesh(self.cfg.rhm_path, self.rhm_train.palm.forward(**drec_rnet)).to(self.dtype).to(self.device)
